 ...
 */
async function generateCourtDocument(inputJsonPath, outputPath, format = 'md') {
    try {
        ...
        if (process.env.OPENAI_API_KEY) {
            ...
            analyzer = new analysis_1.ThreadAnalyzer();
        }
        else {
        ...
        for (let i = 0; i < threads.length; i++) {
            ...
            if (analyzer) {
                try {
                    ...
                    const analysis = await analyzer.analyzeThread(thread, "Provide factual analysis relevant for legal proceedings");
