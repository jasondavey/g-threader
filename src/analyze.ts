...
import { EmailData, EmailThread, AnalysisOptions, ThreadAnalysisResult } from './types';
 ...
 */
async function analyzeEmails(options: AnalysisOptions): Promise<void> {
  try {
    ...
    const filteredThreads = ThreadParser.filterThreads(
      threads,
      options.query,
      options.minMessages,
      options.dateRange,
    ...
    const analyzer = new ThreadAnalyzer(
      undefined, // Use API key from environment
      options.model,
      options.maxTokens
    );
...
async function main() {
  ...
  if (argv.participants) {
    options.participants = argv.participants as string[];
  }
